...
namespace ASPMonitor.Controllers
{
    public class DashBoardController : Controller
    {
        ...
        public DashBoardController(IConfiguration configuration, ILogger<LoginController> logger)
        {
        ...
        public JsonResult removeDashboard([FromBody] List<string> dashboardIds)
        {
            try
            {
                if (dashboardIds == null || dashboardIds.Count == 0)
                {
                    return Json(new { success = false, message = "No dashboard IDs provided." });
                }

                bool result = asp.removeDashboard(dashboardIds);
                ...
                {
                    return Json(new { success = false, message = "Failed to remove dashboards. Database operation returned false." });
                }
            }
            catch (Exception ex)
            {
